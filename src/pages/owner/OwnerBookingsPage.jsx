...
export default function OwnerBookingsPage() {
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      setBookings((prev) =>
        prev.map((b) =>
          b.id === id ? { ...b, bookingStatus: 'Cancel', paymentStatus: 'Refunded' } : b
        )
      );
  ...
  const handleReject = async (id) => {
    ...
    try {
      ...
      setBookings((prev) =>
        prev.map((b) =>
          b.id === id ? { ...b, bookingStatus: 'Booked', paymentStatus: 'Completed' } : b
        )
      );
      if (detailBooking?.id === id) setDetailBooking(null);
    } catch (err) {
  ...
  const cancelRequestCount = bookings.filter((b) => b.bookingStatus === 'Cancel Request').length;
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-6">
        <div>
          ...
          <h2 className="headline-font text-5xl font-extrabold text-on-surface tracking-tighter leading-none">
            Reservations &amp; <span className="text-primary italic">Requests</span>
          </h2>
          <p className="text-on-surface-variant mt-3 max-w-md">
            Quản lý toàn bộ lịch đặt sân. Duyệt hoặc từ chối các yêu cầu hủy từ khách hàng.
          </p>
        </div>
        {cancelRequestCount > 0 && (
          <div className="flex items-center gap-3 bg-amber-500/10 border border-amber-500/30 rounded-xl px-5 py-3">
            ...
            <div>
              <p className="font-bold text-amber-400 text-sm">{cancelRequestCount} Yêu cầu hủy</p>
      ...
      <div className="bg-surface-container-low rounded-xl border border-outline-variant/10 overflow-hidden">
        ...
        {/* Rows */}
        {loading ? (
          <div className="text-center py-16 text-on-surface-variant">Đang tải dữ liệu...</div>
        ) : bookings.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-on-surface-variant">
            <span className="material-symbols-outlined text-5xl opacity-20 mb-4">event_busy</span>
            ...
            >
              ...
              <div className="md:col-span-3 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-on-surface text-sm truncate">{b.customer.name}</p>
                  <p className="text-on-surface-variant text-xs truncate">{b.customer.email}</p>
                </div>
              </div>
              ...
              <div className="md:col-span-3 flex items-center gap-3">
                {b.field.image ? (
                  <img src={b.field.image} alt={b.field.name} className="w-10 h-10 rounded-lg object-cover flex-shrink-0" />
                ) : (
                  <div className="w-10 h-10 rounded-lg bg-surface flex items-center justify-center flex-shrink-0">
                    <span className="material-symbols-outlined text-on-surface-variant/40">stadium</span>
                  </div>
                )}
                <div className="min-w-0">
                  <p className="font-semibold text-on-surface text-sm truncate">{b.field.name}</p>
              ...
              <div className="md:col-span-2">
                ...
                <p className="text-xs text-on-surface-variant mt-0.5">{b.slots?.[0] || '—'}</p>
                {b.slots?.length > 1 && (
                  <p className="text-[10px] text-tertiary">+{b.slots.length - 1} slot</p>
                )}
              </div>
              ...
              {/* Payment Status */}
              <div className="md:col-span-1">
                <span className={`text-xs font-semibold ${paymentStyle[b.paymentStatus] || 'text-on-surface-variant'}`}>
                  {b.paymentStatus}
                </span>
              </div>
              ...
              <div className="md:col-span-2 flex flex-col items-end gap-2">
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${statusStyle[b.bookingStatus] || 'bg-surface text-on-surface-variant'}`}>
                  {b.bookingStatus}
                </span>
      ...
      {detailBooking && (
        <>
          <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm" onClick={() => setDetailBooking(null)} />
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-surface-container-low border border-outline-variant/20 rounded-2xl w-full max-w-lg shadow-2xl">
              <div className="p-6 border-b border-outline-variant/10 flex justify-between items-start">
                <div>
                  <h3 className="headline-font text-xl font-bold">Chi tiết đặt sân</h3>
                  <p className="text-on-surface-variant text-xs mt-1">ID: {detailBooking.id}</p>
                </div>
                <button onClick={() => setDetailBooking(null)} className="text-on-surface-variant hover:text-primary">
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-[10px] uppercase text-on-surface-variant font-label mb-1">Khách hàng</p>
                    <p className="font-semibold">{detailBooking.customer.name}</p>
                    <p className="text-xs text-on-surface-variant">{detailBooking.customer.email}</p>
                    {detailBooking.customer.phone && (
                      <p className="text-xs text-on-surface-variant">{detailBooking.customer.phone}</p>
                    )}
                  </div>
                  <div>
                    <p className="text-[10px] uppercase text-on-surface-variant font-label mb-1">Sân</p>
                    <p className="font-semibold">{detailBooking.field.name}</p>
                    <p className="text-xs text-tertiary">{detailBooking.field.type}</p>
                  </div>
                </div>
                <div>
                  <p className="text-[10px] uppercase text-on-surface-variant font-label mb-2">Ngày & Giờ đặt</p>
                  {detailBooking.slots.map((slot, i) => (
                    <div key={i} className="flex items-center gap-2 text-sm mb-1">
                      <span className="material-symbols-outlined text-sm text-primary">schedule</span>
                      <span>{detailBooking.dates[Math.min(i, detailBooking.dates.length - 1)]} — {slot}</span>
                    </div>
                  ))}
                </div>
                <div className="flex justify-between items-center pt-4 border-t border-outline-variant/10">
                  <div>
                    <p className="text-[10px] uppercase text-on-surface-variant font-label mb-1">Tổng cộng</p>
                    <p className="text-2xl font-bold text-primary headline-font">
                      {detailBooking.totalPrice?.toLocaleString('vi-VN')}đ
                    </p>
                  </div>
                  <div className="text-right">
                    <span className={`px-3 py-1 rounded text-xs font-bold uppercase ${statusStyle[detailBooking.bookingStatus] || ''}`}>
                ...
                {detailBooking.refundReason && (
                  <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3">
                    <p className="text-[10px] uppercase text-amber-400 font-label mb-1">Lý do hủy từ khách</p>
                    <p className="text-sm text-on-surface">{detailBooking.refundReason}</p>
                  </div>
                )}
